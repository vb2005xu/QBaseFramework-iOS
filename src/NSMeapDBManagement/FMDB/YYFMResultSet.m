...
@interface YYFMDatabase ()
- (void)resultSetDidClose:(YYFMResultSet *)resultSet;
@end


@interface YYFMResultSet (Private)
- (NSMutableDictionary *)columnNameToIndexMap;
- (void)setColumnNameToIndexMap:(NSMutableDictionary *)value;
@end

@implementation YYFMResultSet
@synthesize query=_query;
@synthesize columnNameToIndexMap=_columnNameToIndexMap;
@synthesize statement=_statement;

+ (id)resultSetWithStatement:(YYFMStatement *)statement usingParentDatabase:(YYFMDatabase*)aDB {
    
    YYFMResultSet *rs = [[YYFMResultSet alloc] init];
    
    [rs setStatement:statement];
    [rs setParentDB:aDB];
    
    return YYFMDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    YYFMDBRelease(_query);
    _query = nil;
    
    YYFMDBRelease(_columnNameToIndexMap);
...
- (NSDictionary*)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return YYFMDBReturnAutoreleased([dict copy]);
    }
    else {
...
- (void)setParentDB:(YYFMDatabase *)newDb {
    _parentDB = newDb;
}
