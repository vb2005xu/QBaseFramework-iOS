...
@implementation NSMeapDB

/**
 *	@brief	单例
 */
CLASS_SINGLETON_IMPLEMENTATION(NSMeapDB)
...
//        [self createDataBase:NSMEAP_DB_NAME];
//        [self openDataBase:NSMEAP_DB_NAME];
    }
    return self;
}
...
-(BOOL)destroyDBPath:(NSString *)dbPath
{
    ...
    {
        DEBUG_NSLOG(@"destroyDataBase：数据库不存在");
        return YES;
    }
    else
    {
       ...
       [[NSFileManager defaultManager]removeItemAtPath:dbPath error:&error];
        if (error) {
            DEBUG_NSLOG(@"destroyDataBase:%@",error);
            return NO;
        }
        return YES;
    }
...
-(BOOL)createTable:(Class)myClass
{
    ...
    {
        [db executeUpdate:[NSMeapSqlHanldler createTableSql:myClass],nil];
    }];
    return YES;
}
...
-(BOOL)dropTable:(Class)myClass
{
    [self.dbQueue inDatabase:^(YYFMDatabase *db)
     {
         [db executeUpdate:[NSMeapSqlHanldler dropTableSql:myClass],nil];
     }];
    return YES;
//    return [self.db executeUpdate:[NSMeapSqlHanldler dropTableSql:myClass]];
}
...
-(BOOL)insertTable:(Class)myClass dictionary:(NSDictionary *)dictionary
{
    [self.dbQueue inDatabase:^(YYFMDatabase *db)
    {
        ...
        NSString *sql = [NSMeapSqlHanldler insertSql:myClass dictionary:_dictionary];
        [db executeUpdate:sql withParameterDictionary:_dictionary];
    }];
    return YES;
//    NSMutableDictionary *_dictionary=[NSMutableDictionary dictionaryWithDictionary:dictionary];
//    NSString *sql = [NSMeapSqlHanldler insertSql:myClass dictionary:_dictionary];
//    return [self executeWithSql:sql args:_dictionary];
}
    ...
    [self.dbQueue inTransaction:^(YYFMDatabase *db, BOOL *rollback)
     {
         for (NSDictionary *dictionary in array)
         {
             ...
             __block NSString *sql = [NSMeapSqlHanldler insertSql:myClass dictionary:_dictionary];
...
//        NSMutableDictionary *_dictionary=[NSMutableDictionary dictionaryWithDictionary:dictionary];
//        NSString *sql = [NSMeapSqlHanldler insertSql:myClass dictionary:_dictionary];
//        if (![self.db executeUpdate:sql withParameterDictionary:_dictionary])
//        {
//            [self.db rollback];
//            return NO;
//        }
...
-(BOOL)updateTable:(Class)myClass dictionary:(NSDictionary *)dictionary
{
    ...
    {
        NSMutableDictionary *_dictionary=[NSMutableDictionary dictionaryWithDictionary:dictionary];
        NSString *sql = [NSMeapSqlHanldler updateSql:myClass dictionary:_dictionary];
        [db executeUpdate:sql withParameterDictionary:_dictionary];
    }];
    return YES;
//    NSMutableDictionary *_dictionary=[NSMutableDictionary dictionaryWithDictionary:dictionary];
...
- (NSInteger)queryCountFromTable:(Class)myClass conditions:(NSString*)conditions args:(NSArray*)args
{
    ...
    [self.dbQueue inDatabase:^(YYFMDatabase *db)
     ...
     NSString *sql = [NSMeapSqlHanldler queryCountSql:myClass conditions:conditions];
...
{
    NSString *sql = [NSMeapSqlHanldler queryAllSql:myClass conditions:conditions order:order];
...
-(NSArray *)queryObjectFromTable:(Class)myClass conditions:(NSString*)conditions args:(NSArray*)args order:(NSString*)order
{
    ...
    return nil;
//    NSString *sql = [NSMeapSqlHanldler queryAllSql:myClass conditions:conditions order:order];
//    return [self queryObject:myClass sql:sql args:args];
}
...
//    NSString *sql = [NSMeapSqlHanldler queryAllSql:myClass conditions:conditions order:order];
//    return [self queryDictionary:myClass sql:sql args:args];
}
...
{
    NSString *sql = [NSMeapSqlHanldler queryWithPageSql:myClass conditions:conditions pageNumber:pageNumber pageSize:pageSize order:order];
    [self queryObject:myClass sql:sql args:args resultBlock:resultBlock];
}
...
{
    NSString *sql = [NSMeapSqlHanldler queryWithPageSql:myClass conditions:conditions offSet:offSet pageSize:pageSize order:order];
    [self queryObject:myClass sql:sql args:args resultBlock:resultBlock];
}
...
-(BOOL)deleteFromTable:(Class)myClass conditions:(NSString*)conditions args:(NSArray*)args
{
    [self.dbQueue inDatabase:^(YYFMDatabase *db)
    {
        NSString *sql = [NSMeapSqlHanldler deleteSql:myClass conditions:conditions];
        [db executeUpdate:sql withArgumentsInArray:args];
    }];
    
    return YES;
//    NSString *sql = [NSMeapSqlHanldler deleteSql:myClass conditions:conditions];
//    return [self executeWithSql:sql arrayArgs:args];
}

-(NSDictionary *)objectToDictionary:(NSObject *)myObject
{
   NSDictionary *dict = [NSMeapSqlHanldler objectToDictionary:myObject];
    return dict;
}
/**
    ...
    while ([result next])
    {
        ...
        for (NSString *key in dictionary) {
            ...
            if ([obj respondsToSelector:selector]) {
                ...
                if (![method isEqualToString:@"setMeap_id"]) {
